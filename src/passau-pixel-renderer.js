...
export class PassauPixelRenderer {
  ...
  render(snapshot, options = {}) {
    ...
    const alpha = Math.min(1, Math.max(0, Number(options.alpha) || 0)); const player = interpolate(snapshot.player ?? level.actors.player, alpha);
    const cats = (snapshot.cats ?? level.actors.cats).map((cat) => interpolate(cat, alpha)); const elapsed = Number(snapshot.elapsed) || 0;
    ...
    if (options.editor?.cursor) {
      const { x, y, color = 'rgba(245, 196, 81, 0.5)' } = options.editor.cursor;
      scene.fillStyle = color;
      scene.fillRect(x * level.board.tileSize + 2, y * level.board.tileSize + 2, level.board.tileSize - 4, level.board.tileSize - 4);
    }
  ...
  setLevelIfChanged(levelInput) {
    if (this.level === levelInput) return this.level;
    if (this.level?.id === levelInput.id && this.level?.schemaVersion === levelInput.schemaVersion) return this.level;
