...
async function runBenchmarkScenario({
  ...
  try {
    ...
    assert.equal(runtime.result.renderer.contextLost, false, `${name} renderer must not report context loss`);
    assert.deepEqual(runtime.contextLosses, [], `${name} must not lose its WebGL context`);
    ...
    const scenario = {
      ...
      contextLosses: runtime.contextLosses,
      console: messages,
...
async function run() {
  ...
  assert.deepEqual(report.pacer, [
    { refreshRate: 60, presentations: 121 },
    { refreshRate: 120, presentations: 121 },
    { refreshRate: 175, presentations: 121 },
  ]);
